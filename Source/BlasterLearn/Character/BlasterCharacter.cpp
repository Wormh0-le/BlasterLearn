...
void ABlasterCharacter::ReceiveDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatorController, AActor* DamageCauser)
{
	...
	if (AWeapon* DamageWeapon = Cast<AWeapon>(DamageCauser))
	{
		bFriendlyFire = DamageWeapon->bFriendlyFire;
	} else if (AProjectile* DamageProjectile = Cast<AProjectile>(DamageCauser))
	{
		bFriendlyFire = DamageProjectile->bFriendlyFire;
	}
	Damage = BlasterGameMode->CalculateDamage(InstigatorController, Controller, Damage, bFriendlyFire);
...
void ABlasterCharacter::MultiCastElim_Implementation(bool bPlayerLeftGame)
{
	...
	if (ElimBotEffect)
	{
		FVector ElimBotSpawnPoint(GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z + 25.f);
		ElimBotComponent = UGameplayStatics::SpawnEmitterAtLocation(
			GetWorld(), ElimBotEffect,
			ElimBotSpawnPoint,
			GetActorRotation()
		);
	}
...
void ABlasterCharacter::EquipButtonPressed()
{
	...
	if (Combat) {
		...
		if (Combat->CombatState == ECombatState::ECS_Unoccupied && !Combat->bLocallyReloading && bFinishSwapping) ServerEquippedButtonPressed();
		// }
		bool bSwap = Combat->CanSwapWeapons() && !HasAuthority() &&
			Combat->CombatState == ECombatState::ECS_Unoccupied &&
			OverlappingWeapon == nullptr &&
			!Combat->bLocallyReloading && bFinishSwapping;
		if (bSwap)
		{
			PlaySwapMontage();
